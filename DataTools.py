...
class ECGDatasetLoaderv2(Dataset):
    
    def __init__(self,  baseDir='', patients=[], normalize=True, normMethod='0to1', rhythmType='Rhythm', numECGstoFind=1, sex=False):
        self.baseDir = baseDir
        self.rhythmType = rhythmType
        self.normalize = normalize
        self.normMethod = normMethod
    ...
    def findEcgs(self, patient):
        ...
        for ecgIx in range(numberOfEcgs):
            ecgId = str(patientInfo["ecgFileIds"][ecgIx])
            label = patientInfo["gender"][ecgIx] if self.sex else patientInfo["ejectionFraction"]
            zeros = 5 - len(ecgId)
            ecgId = "0"*zeros+ ecgId
            self.fileList.append(os.path.join(patient,
                                        f'ecg_{ecgIx}',
