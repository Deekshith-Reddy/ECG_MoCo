...
def main_worker(gpu, ngpus_per_node, args):
    ...
    scales = args["grid_search"]["params"]["scales"]
    window_ratio = args["grid_search"]["params"]["window_ratio"]
    ...
    for scale in scales:
        for w in window_ratio:
            ...
            print(f"Started Pretaining for {augmentation_name}")



            # Data Loading
            pre_train_sampler, pre_train_loader = dataprep(args, augmentation)
            
            # WandB
            date = datetime.datetime.now().date()

            project = f"MLECG_MoCO_LVEF_PRETRAIN_{augmentation_name}"
            notes = f"Pretrain using {augmentation_name}"
            config = dict(
                batch_size = args["batch_size"],
            ...
            if not args["multiprocessing_distributed"] or (
                ...
                if args["logtowandb"]:
                    wandbrun = wandb.init(
                        ...
                        entity='deekshith',
                        reinit=True,
                        name=f"{networkLabel}_{augmentation_name}_{datetime.datetime.now()}",
                )
                ...
                if not args["multiprocessing_distributed"] or (
                    ...
                    if (epoch + 1)% args["checkpoint_freq"] == 0:
                        save_checkpoint(
                            ...
                            is_best=False,
                            filename=f"{args['checkpoint_dir']}/checkpoint_{augmentation_name}/random_crop_resize/checkpoint_{epoch+1:04d}.pth.tar",
                        )
                    if top1.avg > best_top1:
                        ...
                        filename = f"{args['checkpoint_dir']}/checkpoint_{augmentation_name}/random_crop_resize/checkpoint_best.pth.tar"
